    ...
    def _analyze_user_preferences(self, history):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE ph.user_id = %s
                GROUP BY m.id, m.languages, m.school, m.singer
            ...
            # 计算播放历史的时间衰减因子
            if play_history:
                latest_play_time = max(row[5] for row in play_history if row[5])
                for record in play_history:
                    if record[5]:  # 如果有播放时间
                        # 计算时间衰减权重（越近期的播放权重越大）
                        days_diff = (latest_play_time - record[5]).days
                        time_decay = 1.0 / (1.0 + 0.1 * days_diff)  # 使用指数衰减
                        
                        # 播放次数权重
                        play_weight = min(record[4] / 10.0, 1.0) * time_decay
                        
                        # 更新偏好
                        if record[1]:  # languages
                            preferences['languages'][record[1]] += play_weight
                        if record[2]:  # school
                            preferences['school'][record[2]] += play_weight
                        if record[3]:  # singer
                            preferences['singers'][record[3]] += play_weight
                        
                        preferences['played_music_ids'].add(record[0])
    ...
    def _calculate_recommendation_score(self, music, preferences):
        ...
        try:
            ...
            if music[3] in preferences['languages']:
                base_score += 25 * preferences['languages'][music[3]]

            # 2. 流派匹配度 (25%)
            if music[4] in preferences['school']:
                base_score += 25 * preferences['school'][music[4]]

            # 3. 歌手匹配度 (20%)
            if music[2] in preferences['singers']:
                base_score += 20 * preferences['singers'][music[2]]

            # 4. 播放历史影响 (10%)
            if music[0] in preferences['played_music_ids']:
                base_score += 10
            ...
            # 最终分数：基础分数 + 热度加成（最多20分）
            final_score = base_score + min(popularity_score * 20, 20)
